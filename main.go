...
func main() {
	...
	events := reader.BufferSize() / recordBytes
	...
	var eventsPerSecCopy, eventsPerSecView float64
	if runCopy {
		...
		eventsPerSecCopy = float64(copyCount) / copyDur.Seconds() / 1e6
		fmt.Printf("read-into (copy):     %d events in %s (%.2f Mevents/s), checksum=%d\n",
			copyCount, copyDur, eventsPerSecCopy, copyChecksum)
	}
	...
	if runView {
		...
		eventsPerSecView = float64(zeroCount) / zeroDur.Seconds() / 1e6
		fmt.Printf("read-view (zero-copy): %d events in %s (%.2f Mevents/s), checksum=%d\n",
			zeroCount, zeroDur, eventsPerSecView, zeroChecksum)
	}
	if runCopy && runView {
		// zero copy should be faster than copy, calculate the ratio
		ratio := eventsPerSecView / eventsPerSecCopy
		fmt.Printf("zero-copy is %.2fx faster than copy\n", ratio)
	}
}
